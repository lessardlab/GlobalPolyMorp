#################
## Prepare dataset on any polymorphism 
## Gabriel Muñoz 
## Nov 2021
#################
...
new_id<-paste(ceiling(occ_match$dec_long),
              ceiling(occ_match$dec_lat),
              sep=".")#ceiling function rounds down
...
coords<-as.data.frame(cbind(occ_match$dec_long,
                            occ_match$dec_lat))
...
oceans <-rgdal::readOGR("Data/ne_110m_ocean.shp", 
                        "ne_110m_ocean",
...
coords<-SpatialPointsDataFrame(coords=coords,
                               data=coords,proj4string = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
