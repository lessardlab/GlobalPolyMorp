...
coords <- spTransform(coords,CRSobj = oceans@proj4string )
OcenDAta <- over(coords, oceans)



coords <- coords[which(is.na(OcenDAta$scalerank)),]
plot(coords, pch = ".", col = "red")
plot(cont, add= T)
...
points<-over(cont, coords)#must remove NAs from dataframe
occ_match$continent <- points$continent[match(occ_match$country,points$name)]
occ_match$region_un <- points$region_un[match(occ_match$country,points$name)]

occ_match$uniID <-paste0(occ_match$dec_lat,"_",occ_match$dec_long)
data.frame(coords)
head(occ_match)
occ_clean <- occ_match[match(paste0(coords@coords[,2],"_",coords@coords[,1]), occ_match$uniID),]

plot(occ_clean$dec_lat~occ_clean$dec_long, pch = ".", col = "red")
...
occ_clean<-na.omit(occ_clean)

new_id<-paste(ceiling(occ_clean$dec_long),ceiling(occ_clean$dec_lat),sep=".")#ceiling function rounds down

occ_id<-cbind(occ_clean,new_id)
...
plot(occ_id$dec_lat~occ_id$dec_long,  pch = ".", cex = 0.5 + log(occ_id$Freq), col = ifelse( occ_id$poly_id == 1, scales::alpha("firebrick", 0.7), scales::alpha("skyblue", 0.5)))
plot(cont, add = T, col = scales::alpha("grey", 0.3))
legend("bottomright", c("Poly = 1", "Poly = 0"), fill  = c("firebrick", "skyblue"))
